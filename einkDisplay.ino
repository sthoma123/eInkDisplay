...
// coming from https://github.com/G6EJD/LilyGo-EPD-4-7-OWM-Weather-Display-1
// howto:
// from: https://georgik.rocks/weather-display-for-lilygo-ttgo-t5-4-7-e-paper-esp32-deployed-using-arduino-ide-2-0/
// Flashing:
// Connect the module. Select the board from dropdown in toolbar. Click Port (/dev/ttyACMx on Linux), 
// filter for ESP32 Wrover module and click Ok.
// Click Upload arrow.
// #################################################################################################################
#define LOGGING
...
enum alignment {LEFT, RIGHT, CENTER, TOPLEFT, TOPRIGHT};
#define White         0xFF
...
int  WakeupHour      = 4;  // Don't wakeup until after 07:00 to save battery power
...
//----------------------------------------------
void get_data_hausI(WiFiClient client);
void DisplayHausI(int x, int y);
...
int scanWiFi() { // returns index of known wlan (or -1 if none)
  int scanResult;
  int rv = -1;
  int knownStrength = -99;
    ...
    for (int i = 0; i < scanResult; ++i) {
            ...
            for (int j = 0; j < WifiCount; j++) {
              ...
              if (WiFi.SSID(i) == String(knownSsids[j])) {
                Serial.print ("known: ");
                if (knownStrength < WiFi.RSSI(i)) {
                  Serial.print ("stronger: ");
                  rv = j;
                  knownStrength = WiFi.RSSI(i);
                }
                else {
                  Serial.print ("weaker ");
                }
...
uint8_t StartWiFi(int i) {
  Serial.println("\r\nConnecting to: " + String(knownSsids[i]));
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    notifyUser(String("WiFi ") + knownSsids[i] + " connected at IP: " + WiFi.localIP().toString());
    Serial.println ("MAC Address: " + WiFi.macAddress());
    
  }
  else {
    Serial.printf("WiFi connection %s, %s *** FAILED *** \n",knownSsids[i], knownPasswords[i]);
    notifyUser(String("WiFi connection ")+ knownSsids[i] + "*** FAILED ***");
  }
  return WiFi.status();
}
...
// ---------------------------------------------------------  
// clear little area and display message
void notifyUser(String msg) {
  ...
  epd_clear_area_cycles(area,  6, 50); // cycles / cycles_time
  int cursor_x = 10;
  int cursor_y = 15;  
  writeln((GFXfont *)&OpenSans10B, msg.c_str(), &cursor_x, &cursor_y, NULL);
...
void setup() {
  ...
  WifiState = WL_IDLE_STATUS;

  int foundWifi = scanWiFi();

  if (foundWifi > -1) {
      WifiState = StartWiFi(foundWifi);
  }
  else
     notifyUser(String("No known WiFi found"));

  
  if (WifiState == WL_CONNECTED && SetupTime() == true) {
    bool WakeUp = false;                
    ...
    if (WakeUp) {
      ...
      get_data_hausI(client);
      notifyUser("Received HausI data ...");
      ...
      notifyUser("Received all weather data");
...
bool obtainWeatherData(WiFiClient & client, const String & RequestType) {
  ...
  // Serial.printf("URI IS>>%s<<", uri.c_str());
...
boolean UpdateLocalTime() {
  ...
  while (!getLocalTime(&timeinfo, 5000)) { // Wait for 5-sec for time to synchronise
    ...
    notifyUser("Failed to obtain time");
    return false;
  }
...
void drawString(int x, int y, String text, alignment _align) {
...
/////////////////// code domotique  //////////////////////////////////////////////////////////////
/// reste du code de traitement dans hausI.h 

//----------------------------------------------
void drawMeas (int x, int y, measure_type meas) {
    
    setFont(OpenSans12B);
    drawString(x,                         y-2, meas.bez, TOPLEFT);
    
    setFont(OpenSans18B); 
    drawString(x + COL1WIDTH + COL2WIDTH, y, meas.val, TOPRIGHT); 
    
    setFont(OpenSans12B);
    drawString(x + COL1WIDTH + COL2WIDTH + 3, y, meas.unit, TOPLEFT); //Etwas Abstand zum Wert
}
...
void DisplayHausI(int x, int y) {
  ...
  y+=LINEHEIGHT;
  drawMeas (x,y, CurrHausIData.PHouse);

  y+=LINEHEIGHT;
  drawMeas (x,y, CurrHausIData.OutTemp);  y+=LINEHEIGHT;
  ...
  //drawString(x, y+18+2, "Temp " + String(CurrHausIData.OutTemp, 1) + " °C", LEFT); 
  
}
//----------------------------------------------

/// hausI
const char * host = "192.168.0.183";  // kellerraspi as proxy
int httpPort = 80; 

// max values
const int max_val=12;
//const String hausI_IDX[max_val][3] = {{"Ballon","61","Data"},{"Batterie","83","Data"},{},{"Linky","28","Usage"},{"Today","28","CounterToday"},{},{"Solaire","29","Usage"},{"Today","29","CounterToday"},{},{"Gazpar","55","CounterToday"}} ; 
String hausI_result[max_val]; 
...
//////////////////////////// fin code domotique  //////////////////////////////////////////////////////////////
/////////////////// get "data" in json
String mapUnit(const char * srcUnit) {
  String rv = srcUnit;

  if (0 == strcmp(" Grad", srcUnit)) {
    rv = "°C";
  }
...
void mapVal(String & val, String & unit) {
  float p;

  if (String(" Grad") == unit) {
    val = String (val.toFloat(),1);
  }
  if (String("%")== unit) {  //convert no decimals
    p = val.toFloat();
    val = String(p,0);
  }
  
  if (0 == strcmp("W", unit.c_str())) {  //convert to kW
    p = val.toFloat();
    if (fabs (p) > 999 ) { //tokW
      val = String(p/1000,2);
      unit = "kW";
...
//----------------------------------------------------------------------------------------------------
void hausIDecodeLine( const char * dp, JsonVariant  jsonLine, measure_type * measure, String bez) {
  //JsonVariant jsonLine;
  String unit;
  String val;

  if (0 == strcmp(dp, jsonLine[4])) {  
    measure->bez=bez;
    if (0 == strcmp("Ok", jsonLine[5])) {
      unit = String(jsonLine[2].as<const char *>());
      val = "unk_type";
      if (jsonLine[1].is<const char*>()) {
        val = String(jsonLine[1].as<const char *>());
      }
      
      if (jsonLine[1].is<float>()) {
        val = String(jsonLine[1].as<float>(),1);
      }
      Serial.printf(val.c_str());
      Serial.printf(" = ");
      Serial.println(unit.c_str());
      mapVal(val, unit);

      
      measure->val = val;
      measure->unit = unit;
      measure->unit=mapUnit(unit.c_str());
...
//--------------------------------------------------------------------------------------------------
// writes result into global structure HausIData. (empty it if no response)
// returns false on error.
bool hausIDecoderesult(WiFiClient& json) {
  String line;
  String dummy;

  //Serial.println(F("\nCreating JsonObject"));
  dummy = json.readStringUntil('\n'); //discard last line containing size.
  //Serial.println("nignoring:");
  Serial.println(dummy);
  Serial.println("JSONData: ");
  line = json.readStringUntil('\n'); 
  Serial.println(line);

  Serial.println("AdditionalData: ");
  String line2 = json.readString();
  Serial.println(line2);
    
  DynamicJsonDocument doc(64 * 512);                      // allocate the JsonDocument
  DeserializationError error = deserializeJson(doc, line.c_str()); // Deserialize the JSON document
  if (error) {                                             // Test if parsing succeeds.
    Serial.print(F("deserializeJson() failed: "));
    Serial.println(error.c_str());
    return false;
  ...
  serializeJsonPretty(doc, line);
  Serial.print(F("Json is: "));
  Serial.println(line); //print pretty json
  
  // convert it to a JsonObject
  JsonObject root = doc.as<JsonObject>();
  Serial.println("Decoding HausI data");
...
bool hausIGetJsonData(WiFiClient& client, const char* host, const char* httpPort , String box, String dps){
  ...
  //http://kr/secureJSONProxy.php?http://raspi:8000/readMultiJ?dp=%5B%22BX/BATTERX/353%22,%22BX/BATTERX/1634%22,%22BX/BATTERX/1121%22,%22BX/BATTERX/1074%22,%22BX/BATTERX/GRIDPOWER%22%5D
   //const String TEMP/00042d9aabff httpPort
  // String url = "https://"+String(host)+":"+String(8443)+"/secureJSONProxy.php?http://"+box+":8000/readMultiJ?dp=%5B"+dps+"%5D";
   String UID = WiFi.macAddress(); //currently mac address;

   
   String url = "/secureJSONProxy.php?http://"+box+":8000/readMultiJ?dp=%5B"+dps+"%5D";
//
    //Serial.printf(url.c_str());

  for (int i = 3;  i; i--) {  //retries since raspberries are slow sometimes:
    Serial.printf("try %d: ", i);
    // bool begin(WiFiClient &client, String host, uint16_t port, String uri = "/", bool https = false);
    //http.begin(url, certificate_home); //http.begin(uri,test_root_ca); //HTTPS example connection
    
    http.begin(host, 8443, url, certificate_home);
    // "Cookie:" + COOKIE_NAME + "=" + cookieValue + "\r\n"
    http.addHeader("Cookie", "ClientUID=" + UID);

    int httpCode = http.GET();
    if (httpCode == HTTP_CODE_OK) {
      Serial.printf("http code Ok");
      rv = hausIDecoderesult(http.getStream());
      break;
    }
    else
    {
      if (i>0) Serial.printf("retry %d: ", i);
      else Serial.printf("end of retries, giving up ");
      
      Serial.printf("connection to %s:%d failed, error: %d, %s\n", host, httpPort, httpCode, http.errorToString(httpCode).c_str());
      rv = false;
      delay(1000);
    }
